...
__all__ = ["extract_seq", "extract_aligned_seq", "extract_aligned_directed_seq"]
...
def extract_aligned_directed_seq(label0_dirs, label1_dirs, aligned_id_dict, seq_length_range):
	...
	aligned_id_set = set(aligned_id_dict.keys())
	...
	_label_direct_seq(sequences, labels, label0_seq, "0", aligned_id_dict, label0_keys, seq_length_range)
	_label_direct_seq(sequences, labels, label1_seq, "1", aligned_id_dict, label1_keys, seq_length_range)
...
def _label_direct_seq(out_sequences, out_labels, in_sequences, in_label, in_direct_dict, in_keys, seq_length_range):

	for key in in_keys:
		if (seq_length_range[0] < len(in_sequences[key])) or (len(in_sequences[key]) <= seq_length_range[1]):
			out_sequences[key] = in_sequences[key]
			out_labels[key] = in_label + in_direct_dict[key]
